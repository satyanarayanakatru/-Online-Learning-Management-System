...
export default function DeleteConfirmModal({ isOpen, course, onClose, onConfirmDelete }) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md">
      <div className="teal-glass-panel w-full max-w-md rounded-3xl p-6 space-y-5 shadow-2xl relative border border-rose-500/30 text-center">
        ...
        <div className="space-y-1">
          <h3 className="text-xl font-black text-white">Delete Course?</h3>
          <p className="text-xs text-emerald-200/70 font-semibold">
            Are you sure you want to delete <strong className="text-amber-300">"{course.title}"</strong>? This action cannot be undone.
          </p>
        </div>
        ...
        <div className="flex justify-center space-x-3 pt-2 border-t border-emerald-500/15">
          <button
            onClick={onClose}
            className="px-4 py-2.5 text-xs font-bold text-emerald-300 hover:text-white bg-emerald-500/10 hover:bg-emerald-500/20 rounded-xl transition"
          >
            Cancel
          </button>
          <button
